...
def extract_image_patch(image, bbox, patch_shape):
    ...
    image : ndarray
        The full image.
    bbox : array_like
        The bounding box in format (x1,y1,x2,y2) instead of #(x, y, width, height) as the original.
...
def create_box_encoder(model_filename, input_name="images",
    ...
    def encoder(image, boxes):
        ...
        for box_xywh in boxes_xywh:
            box_xywh[2:] = box_xywh[2:]-box_xywh[:2] # to convert xhwh
            patch = extract_image_patch(image, box_xywh, image_shape[:2])
            if patch is None:
                print("WARNING: Failed to extract image patch: %s." % str(box_xywh))
