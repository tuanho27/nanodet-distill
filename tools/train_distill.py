...
from nanodet.trainer.distill_task import TrainingDistillTask
...
def main(args):
    ...
    task = TrainingDistillTask(cfg, evaluator)

    if "load_model" in cfg.schedule:
        ckpt = torch.load(cfg.schedule.load_model, map_location=f"cpu")
    ...
    if "load_teacher_model" in cfg.schedule:
        ckpt = torch.load(cfg.schedule.load_teacher_model,map_location=f"cpu")
        if "pytorch-lightning_version" not in ckpt:
            warnings.warn(
                "Warning! Old .pth checkpoint is deprecated. "
                "Convert the checkpoint with tools/convert_old_checkpoint.py "
            ...
            ckpt = convert_old_model(ckpt)
        load_model_weight(task.teacher_model, ckpt, logger)
        logger.info("Loaded teacher model weight from {}".format(cfg.schedule.load_teacher_model))
